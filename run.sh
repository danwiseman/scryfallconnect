...
if hash docker 2>/dev/null; then
    # for docker lovers
    docker build . -t danwiseman/scryfallconnect:1.0
    docker run --net=host --rm -t \
           -v $(pwd)/offsets:/scryfallconnect/offsets \
           danwiseman/scryfallconnect:1.0
    ...
    $KAFKA_HOME/bin/connect-standalone.sh $KAFKA_HOME/etc/schema-registry/connect-avro-standalone.properties config/ScryfallSourceConnecter.properties
elif [[ -z $CONFLUENT_HOME ]]; then
    # for people who installed kafka confluent flavour
    $CONFLUENT_HOME/bin/connect-standalone $CONFLUENT_HOME/etc/schema-registry/connect-avro-standalone.properties config/ScryfallSourceConnecter.properties
else
